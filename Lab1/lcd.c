...
void lcd_rotatingBanner(char data[]){
    ...
    //Add 19 spaces so that we start on the right side
    int bufLen = (len > (LCD_WIDTH + 1)) ? len + 19 : LCD_WIDTH + 19;
    ...
    while(true){
        ...
        char message[LCD_WIDTH + 1];

        strncpy( message, val, LCD_WIDTH);
